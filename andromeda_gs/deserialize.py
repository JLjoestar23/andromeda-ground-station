...
class DataHandler:
    def __init__(self):
        ...
        self.buffer = (
            []
        )  # Initialize a buffer so that self.data doesn't get overwritten when updating. This accumulates data packets
        self.HOST = "0.0.0.0"
        self.PORT = 4210
        self.s = None
    ...
    def establish_connection(self):
        try:
            self.s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # Use self.s
            self.s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            self.s.bind((self.HOST, self.PORT))
            self.s.settimeout(3)

            # Check if the socket is properly set up by attempting to receive data
            try:
                self.s.recvfrom(
                    1024
                ...
                return "Connected (no data recieved)"  # Connection established but no data received yet
    ...
    def receive_data(self):
        try:
            self.data, addr = self.s.recvfrom(
                1024
            )  # _ = addr which we don't care about
            print(self.data, addr)  # printing for debug purposes
            if not self.data:
                return "No data coming through"
                # print(
                #     "There is no data coming through"
                # )  ### THIS IS A SCAFFHOLD - WHAT IS PROPER EXIT CONDITION?
            self.buffer.append(self.data)
        except socket.timeout:
            return "Timeout: No data received in last 10 seconds"  # print("Timeout: No data received in last 10 seconds.")
    ...
    def split_data_list(self):
        ...
        for data_packet in self.buffer:
            decoded_string = data_packet.decode(
                "utf-8"
            )  # because we're receiving a byte string we need to decode with a format
            actual_list = ast.literal_eval(
                decoded_string
            )  # evaluate the string so it becomes a list of this decoded string
            length_to_split = [1] * len(
                actual_list
            )  # split the input array at each data point
    ...
    def convert_to_float(self, output):
        ...
        flatten_list = [
            flat for item in output for flat in item
        ]  # return only the string in the flattened list
